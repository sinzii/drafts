...
- Compatible with `@polkadot/extension` API
    ...
    - While the above approach is absolutely feasible, during development we found that this approach requires users to disable the `Block third-party cookies` setting in their browser in order for the embed iframe of Coong Wallet to work properly so we can access granted information & subscribe to changes in the wallet. Since when users enable the `Block third-party cookies` setting in the browser, it will also block access to `localStorage` inside the embed iframe which make it impossible for the dapps (sdk) to access wallet information.
      - Requires users to disable the `Block third-party cookies` setting in their browser could make the website-based wallet experience (API, UX) almost like extension-based wallets, but this also posing some tradeoffs:
        - This would violate users' privacy since users probably don't want to disable this setting and getting tracked by social media platforms and other services.
        - Each browsers (both on mobile & desktop) have their own way of turning on/off this setting, so it would need to have a custom instructions to helps user to disable the setting which make complicated for users to follow before they can use the wallet. Since what we want to do is to make the onboarding new users experience more friendly, so we would want avoid any complicated instructions.
      - With that, we decided to take a different approach to address this issue:
        - No longer initiate an embed iframe of the wallet inside the dapp, so we don't have to deal the `Block third-party cookies` issue any more.
        - Using first-party localStorage (dapp's localStorage) to store granted information (connected accounts) and introducing extra APIs to (1) allow users to add/remove (update) their granted information to the dapp, (2) clear granted information when users want to sign out of the dapp. With this approach, the process of integrate Coong Wallet into dapp would need to have some extra handling for developers, but for users, they don't have to turn on or off any browser settings.
- Security first
